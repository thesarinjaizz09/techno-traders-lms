...
import { useMessages, useMessagesCache, usePrivateMessages, usePrivateMessagesCache } from "../hooks/use-forum";
...
export default function PrivateForum() {
    ...
    const { data: serverMessages, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = usePrivateMessages()
    ...
    const [activeChannel, setActiveChannel] = useState(channels[1]?.id ?? "private-community");
    ...
    useEffect(() => {
        ...
        socket.on("private:message:new", onMessageNew);
        socket.on("private:message:system", onMessageSystem);

        return () => {
            socket.off("private:message:new", onMessageNew);
            socket.off("private:message:system", onMessageSystem);
        };
    }, [socket, cache]);
    ...
    useEffect(() => {
        ...
        socket.on("private:typing:start", onTypingStart);
        socket.on("private:typing:stop", onTypingStop);

        return () => {
            socket.off("private:typing:start", onTypingStart);
            socket.off("private:typing:stop", onTypingStop);
        };
    }, [socket]);
    ...
    const sendMessage = () => {
        ...
        socket.emit("private:message:send", { content: composer, clientMessageId: tempId }, (ack: any) => {
            if (!ack?.success) {
